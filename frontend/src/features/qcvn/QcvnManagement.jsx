...
function formFromConfig(config) {
  return {
    station_ids: (config.station_ids || []).map(String),
...
function payloadFromForm(form) {
  ...
  return {
    station_ids: form.station_ids.map(Number),
    metric: form.metric,
...
export default function QcvnManagement({ currentUser, onStationsChanged, stations }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setModalOpen(false);
      setMessage({ tone: "success", text: editingConfig ? "Đã cập nhật QCVN" : "Đã thêm QCVN" });
      await load();
  ...
  async function handleDelete(config) {
    if (!window.confirm(`Xóa QCVN ${metricLabels[config.metric]}?`)) return;
    try {
    ...
    <section className="space-y-4">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">QCVN độc lập; có thể gán thủ công cho nhiều trạm.</p>
        </div>
      ...
      {stations.length === 0 && <p className="rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-800">Chưa có trạm; QCVN vẫn có thể tạo trước và gán sau.</p>}

      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase text-slate-500">
              <tr><th className="px-4 py-3">Trạm áp dụng</th><th className="px-4 py-3">Thông số</th><th className="px-4 py-3">Cảnh báo</th><th className="px-4 py-3">Nghiêm trọng</th><th className="px-4 py-3">Trạng thái</th><th className="px-4 py-3 text-right">Hành động</th></tr>
            </thead>
            <tbody>
              {loading ? <tr><td className="px-4 py-8 text-center text-slate-500" colSpan="6">Đang tải cấu hình QCVN...</td></tr> : configs.length === 0 ? <tr><td className="px-4 py-8 text-center text-slate-500" colSpan="6">Chưa có QCVN nào được gắn cho trạm.</td></tr> : configs.map((config) => (
                <tr className="border-t border-slate-100" key={config.id}>
                  <td className="px-4 py-3"><p className="font-medium text-slate-900">{(config.station_ids || []).map((id) => stations.find((station) => String(station.id) === String(id))?.code || `Trạm #${id}`).join(", ") || "Chưa gán trạm"}</p></td>
      ...
      {modalOpen && <div className="fixed inset-0 z-40 grid place-items-center bg-slate-950/50 p-4"><form className="w-full max-w-2xl rounded-lg bg-white shadow-xl" onSubmit={handleSubmit}><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><h2 className="text-lg font-semibold">{editingConfig ? "Sửa QCVN" : "Thêm QCVN"}</h2><button className="rounded-md p-2 text-slate-500" onClick={() => setModalOpen(false)} type="button"><X className="h-5 w-5" /></button></div><div className="grid gap-3 p-5 md:grid-cols-2">
        <fieldset className="grid gap-2 text-sm md:col-span-2"><legend>Trạm áp dụng (có thể chọn nhiều)</legend><div className="grid max-h-40 gap-2 overflow-y-auto rounded-md border border-slate-300 p-3 md:grid-cols-2">{stations.map((station) => <label className="flex items-center gap-2" key={station.id}><input checked={form.station_ids.includes(String(station.id))} onChange={(event) => updateField("station_ids", event.target.checked ? [...form.station_ids, String(station.id)] : form.station_ids.filter((id) => id !== String(station.id)))} type="checkbox" />{station.code} - {station.name}</label>)}{stations.length === 0 && <span className="text-slate-500">Chưa có trạm</span>}</div></fieldset>
