...
const blankForm = { name: "", host: "", port: 21, user: "", password: "", root_path: "/data", timeout_seconds: 5 };
...
export default function FtpManagement({ currentUser, stations }) {
 ...
 const [message, setMessage] = useState(null);
  ...
  function openEdit(config) {
    ...
    setForm({ name: config.name, host: config.host, port: config.port, user: config.user, password: "", root_path: config.root_path, timeout_seconds: config.timeout_seconds });
    setMessage(null);
    setModalOpen(true);
  }
 ...
 function updateField(field, value) { setForm((current) => ({ ...current, [field]: value })); }
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white"><table className="w-full min-w-[820px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Tên FTP</th><th className="px-4 py-3">Máy chủ</th><th className="px-4 py-3">User</th><th className="px-4 py-3">Thư mục gốc</th><th className="px-4 py-3 text-right">Hành động</th></tr></thead><tbody>
        {loading ? <tr><td className="px-4 py-8 text-center text-slate-500" colSpan="5">Đang tải danh mục FTP...</td></tr> : configs.length === 0 ? <tr><td className="px-4 py-8 text-center text-slate-500" colSpan="5">Chưa có FTP. Hãy tạo FTP trước khi thêm trạm.</td></tr> : configs.map((config) => <tr className="border-t border-slate-100" key={config.id}><td className="px-4 py-3"><strong>{config.name}</strong></td><td className="px-4 py-3 font-mono">{config.host}:{config.port}</td><td className="px-4 py-3">{config.user}</td><td className="px-4 py-3 font-mono text-xs">{config.root_path}</td><td className="px-4 py-3"><div className="flex justify-end gap-2"><button className="rounded-md border border-slate-200 p-2" onClick={() => openBrowser(config)} title="Xem file" type="button"><Eye className="h-4 w-4" /></button><button className="rounded-md border border-slate-200 p-2" onClick={() => openEdit(config)} title="Sửa" type="button"><Edit2 className="h-4 w-4" /></button><button className="rounded-md border border-red-200 p-2 text-red-600" onClick={() => handleDelete(config)} title="Xóa" type="button"><Trash2 className="h-4 w-4" /></button></div></td></tr>)}
      </tbody></table></div>
      ...
      {modalOpen && <div className="fixed inset-0 z-40 grid place-items-center bg-slate-950/50 p-4"><form className="w-full max-w-3xl rounded-lg bg-white shadow-xl" onSubmit={handleSubmit}><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><h2 className="text-lg font-semibold">{editingFtpId ? "Sửa FTP" : "Thêm FTP độc lập"}</h2><button className="rounded-md p-2 text-slate-500" onClick={() => setModalOpen(false)} type="button"><X className="h-5 w-5" /></button></div><div className="grid gap-3 p-5 md:grid-cols-2">
        ...
        <p className="text-xs text-slate-500 md:col-span-2">Sau khi lưu FTP, vào <strong>Quản lý trạm</strong> để chọn đúng folder dữ liệu cho từng trạm.</p>
